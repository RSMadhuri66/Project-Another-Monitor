...
public class ActivityMain extends Activity implements ActivityCompat.OnRequestPermissionsResultCallback {
	...
	private final DecimalFormat mFormatPercent = new DecimalFormat("##0.00");
	private final DecimalFormat mFormatTime = new DecimalFormat("0.##");
...
// private ScriptIntrinsicBlur intrinsic;
	...
	private final Handler mHandlerVG = new Handler();
	...
	private final Runnable drawRunnable = new Runnable() {
		...
		@Override
		public void run() {
			mHandler.postDelayed((Runnable) this, (long) intervalUpdate);
	...
	private final Runnable drawRunnableGraphic = new Runnable() { // http://stackoverflow.com/questions/18856376/android-why-cant-i-create-a-handler-in-new-thread
		@Override
		public void run() {
			mThread = new Thread() {
				@Override
 ...
 break;
	...
	private final BroadcastReceiver receiverSetIconRecord = new BroadcastReceiver() {
		@Override
		public void onReceive(Context context, Intent intent) {
			setIconRecording();
		}
	};
	private final BroadcastReceiver receiverDeadProcess = new BroadcastReceiver() {
		@SuppressWarnings("unchecked")
		@Override
		public void onReceive(Context context, Intent intent) {
...
// yBottom = (int) (getHeight()*0.88);
		...
		final TextView mTVIntervalRead;
		mTVIntervalRead = (TextView) findViewById(R.id.TVIntervalRead);
		mTVIntervalRead.setText(getString(R.string.interval_read) + " " + decimalFormat.format(intervalRead/1000.00) + " s");


		final TextView mTVIntervalUpdate;
		mTVIntervalUpdate = (TextView) findViewById(R.id.TVIntervalUpdate);
		mTVIntervalUpdate.setText(getString(R.string.interval_update) + " " + decimalFormat.format(intervalUpdate/1000.00) + " s");
		final TextView mTVIntervalWidth;
		mTVIntervalWidth = (TextView) findViewById(R.id.TVIntervalWidth);

		mTVIntervalWidth.setText(getString(R.string.interval_width) + " " +decimalFormat.format(intervalWidth/1000.00) + " dp");

		mSBRead = (SeekBar) findViewById(R.id.SBIntervalRead);
		mSBRead.setMax(5); // Adjust the maximum value based on the number of frequencies
		int t = 0;
		switch ((int) intervalRead) {
			case 250: t = 0; break;
			case 500: t = 1; break;
			case 1000: t = 2; break;
			case 2000: t = 3; break;
...
// mTVIntervalRead.setText(getString(R.string.interval_read) + " " + mFormatTime.format(t/(float)1000) + " s");
				DecimalFormat decimalFormat = new DecimalFormat("#.##");
				mTVIntervalRead.setText(getString(R.string.interval_read) + " " + decimalFormat.format((float)t/1000.00) + " s");
			}
		});
		...
		mSBUpdate.setOnSeekBarChangeListener(new SeekBar.OnSeekBarChangeListener() {
			...
			public void onProgressChanged(SeekBar seekBar, int progress, boolean fromUser) {
				...
				mTVIntervalUpdate.setText(getString(R.string.interval_update) + " " + decimalFormat.format((float)t/1000.00) + " s");
			}
		});

		t = 0;
		switch ((int) intervalWidth) {
...
// mWidth.setText(getString(R.string.interval_width) + " " + (double)t/1000 + " dp")
				...
				mTVIntervalWidth.setText(getString(R.string.interval_width) + " " + decimalFormat.format(((float)t/1000.00)) + " dp");
			}
		});
		...
		mCloseSettings.setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View v) {
				...
				double intervalRead = 0;
				double intervalUpdate = 0;
				...
				switch (mSBRead.getProgress()) {
					...
					case 5: intervalRead = 8000; break;
				}
				...
				switch (mSBUpdate.getProgress()) {
					...
					case 5: intervalUpdate = 8000; break;
				}

				switch (mSBWidth.getProgress()) {
					case 0: intervalWidth =  0.25; break;
					case 1: intervalWidth =  0.5; break;
					case 2: intervalWidth = 1; break;
					case 3: intervalWidth = 2; break;
					case 4: intervalWidth = 4; break;
					case 5: intervalWidth = 8; break;
					default: intervalWidth = 0.0; break;
				}
				//making changes stated with lower bound 500 - 2000 UB
				...
				ActivityMain.this.intervalRead = (int) intervalRead;
				ActivityMain.this.intervalUpdate = (int) intervalUpdate;
				ActivityMain.this.intervalWidth = (int) intervalWidth;

				mSR.setIntervals((int) intervalRead, (int) intervalUpdate, (int) intervalWidth);
				mVG.calculateInnerVariables();
				mHandlerVG.post(drawRunnableGraphic);
				mHandler.removeCallbacks(drawRunnable);
				mHandler.post(drawRunnable);
				mPrefs.edit()
						.putInt(C.intervalRead, (int) intervalRead)
						.putInt(C.intervalUpdate, (int) intervalUpdate)
						.putInt(C.intervalWidth, (int) intervalWidth)
						.apply();
			}
		});
